<#
.SYNOPSIS
    Syncs the TSRAllEmployees AD group — Toyota & Lexus Sales / Showroom division.
...
.PARAMETER Diagnose      Report TSR dept users NOT in AllEKKEmployees.
#>
param(
    [switch]$Test,
    [switch]$Auto,
...
$Departments = @(
    ...
    "Marketing - TLS"
    "Material Handling - TLS"
    "Toyota and Lexus Sales"
    "Toyota Showroom"
    "Vehicle Ordering - TLS"
)
# ══════════════════════════════════════════════════════════════════════════════
...
Write-Log "================================================================" "HEADER"
Write-Log "  TSRAllEmployees Group Sync  —  $(Get-Date -Format 'dd-MMM-yyyy HH:mm')" "HEADER"
...
function Get-ADParams { $p = @{}; if ($adCred) { $p['Credential'] = $adCred }; return $p }

foreach ($grp in @($SourceGroup, $TargetGroup)) {
    try { Get-ADGroup -Identity $grp @(Get-ADParams) -ErrorAction Stop | Out-Null; Write-Log "Group verified: $grp" }
    catch { Write-Log "Group '$grp' not found: $_" "ERROR"; exit 1 }
}

# Step 1 — Source members filtered by dept
Write-Log "--- Step 1: Reading $SourceGroup members ---"
try {
    $allEKK = Get-ADGroupMember -Identity $SourceGroup -Recursive @(Get-ADParams) |
        Where-Object { $_.objectClass -eq 'user' } |
        ForEach-Object { Get-ADUser -Identity $_.DistinguishedName @(Get-ADParams) -Properties DisplayName,Department,SamAccountName }
...
# Step 2 — Current target group members
Write-Log "--- Step 2: Reading current $TargetGroup members ---"
try {
    $current    = Get-ADGroupMember -Identity $TargetGroup -Recursive @(Get-ADParams) | Where-Object { $_.objectClass -eq 'user' } |
        ForEach-Object { Get-ADUser -Identity $_.DistinguishedName @(Get-ADParams) -Properties DisplayName,Department,SamAccountName }
...
if ($toAdd.Count -eq 0 -and $toRemove.Count -eq 0) {
    ...
    if ($toAdd.Count -gt 0) {
        ...
        if ($Test) { $toAdd | Sort-Object DisplayName | ForEach-Object { Write-Log ("  [DRY-RUN ADD] {0} ({1}) [{2}]" -f $_.DisplayName,$_.SamAccountName,$_.Department) }; $addedOK=$toAdd.Count }
            ...
            try {
                Add-ADGroupMember -Identity $TargetGroup -Members ($toAdd|Select-Object -ExpandProperty DistinguishedName) @(Get-ADParams) -ErrorAction Stop
                ...
                foreach ($u in $toAdd|Sort-Object DisplayName) {
                    try { Add-ADGroupMember -Identity $TargetGroup -Members $u.DistinguishedName @(Get-ADParams) -ErrorAction Stop; Write-Log "  [ADDED] $($u.DisplayName)" "SUCCESS"; $addedOK++ }
    ...
    if ($toRemove.Count -gt 0) {
        Write-Log "--- Removing $($toRemove.Count) users (dept no longer matches) ---"
        foreach ($u in $toRemove|Sort-Object DisplayName) {
            ...
            if ($Test) { Write-Log "  [DRY-RUN REMOVE] $label" "WARN"; $removedOK++ }
                ...
                try { Remove-ADGroupMember -Identity $TargetGroup -Members $u.DistinguishedName @(Get-ADParams) -Confirm:$false -ErrorAction Stop; Write-Log "  [REMOVED] $label" "WARN"; $removedOK++ }
    ...
    if ($Test) { Write-Log ("  Would ADD           : {0}" -f $addedOK) "SUCCESS"; Write-Log ("  Would REMOVE        : {0}" -f $removedOK) "WARN" }
        ...
        Write-Log ("  ADDED               : {0}" -f $addedOK) "SUCCESS"
        Write-Log ("  REMOVED             : {0}" -f $removedOK) "WARN"
...
if ($Diagnose) {
    Write-Log "================================================================" "HEADER"
    Write-Log "  DIAGNOSTIC: Finding dept users NOT in $SourceGroup" "HEADER"
    $ekk_DNs = $allEKK | Select-Object -ExpandProperty DistinguishedName
    $allInAD = @(); foreach ($d in $Departments) { $allInAD += Get-ADUser -Filter "Department -eq '$d' -and Enabled -eq `$true" -Properties DisplayName,Department,SamAccountName @(Get-ADParams) }
    $allInAD = $allInAD | Sort-Object DistinguishedName -Unique
    $missing = $allInAD | Where-Object { $ekk_DNs -notcontains $_.DistinguishedName }
    Write-Log ("  Total in AD with matching dept : {0}" -f $allInAD.Count)
