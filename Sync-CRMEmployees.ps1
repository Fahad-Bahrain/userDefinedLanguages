<#
.SYNOPSIS
    Syncs the CRM Division Group AD group based on Department membership.
...
.PARAMETER Diagnose      Report CRM dept users NOT in AllEKKEmployees.
...
$TargetGroup = "CRM Division Group"
$LogDir      = "C:\AD-MailSync\Logs"
$LogFile     = Join-Path $LogDir ("CRMEmployees_Sync_{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss"))

$CRM_Departments = @(
    "Administration and Support - CRM"
    "Customer Engagement Centre - CRM"
...
Write-Log "================================================================" "HEADER"
Write-Log "  CRM Division Group Sync  -  $(Get-Date -Format 'dd-MMM-yyyy HH:mm')" "HEADER"
...
$crmFiltered = $allEKK | Where-Object { $dept=$_.Department; $CRM_Departments | Where-Object { $_ -ieq $dept } }
Write-Log ("Users matching CRM departments: {0}" -f $crmFiltered.Count) "INFO"
if ($crmFiltered.Count -eq 0) { Write-Log "No users matched. Verify Department values in AD." "WARN"; exit 0 }
Write-Log "Department breakdown:"
$crmFiltered | Group-Object Department | Sort-Object Count -Descending | ForEach-Object { Write-Log ("  {0,4}  {1}" -f $_.Count,$_.Name) }

Write-Log "--- Step 2: Reading current $TargetGroup members ---" "INFO"
try {
    $currentCRM = Get-ADGroupMember -Identity $TargetGroup -Recursive @adParams |
        Where-Object { $_.objectClass -eq 'user' } |
        ForEach-Object { Get-ADUser -Identity $_.DistinguishedName @adParams -Properties DisplayName,Department,EmailAddress,SamAccountName }
    $currentDNs = $currentCRM | Select-Object -ExpandProperty DistinguishedName
    Write-Log ("Current members in {0}: {1}" -f $TargetGroup, $currentCRM.Count)
} catch { Write-Log "Failed to read '$TargetGroup': $_" "ERROR"; exit 1 }

Write-Log "--- Step 3: Calculating changes ---" "INFO"
$toAdd          = $crmFiltered | Where-Object { $currentDNs -notcontains $_.DistinguishedName }
$toRemove       = $currentCRM  | Where-Object { $dept=$_.Department; -not ($CRM_Departments | Where-Object { $_ -ieq $dept }) }
$alreadyCorrect = $crmFiltered | Where-Object { $currentDNs -contains $_.DistinguishedName }
...
if ($toRemove.Count -gt 0) {
    Write-Log "--- Removing users (department no longer CRM) ---" "INFO"
...
if ($Diagnose) {
    Write-Log "================================================================" "HEADER"
    Write-Log "  DIAGNOSTIC: CRM dept users NOT in $SourceGroup" "HEADER"
    $ekk_DNs = $allEKK | Select-Object -ExpandProperty DistinguishedName
    $allInAD = @(); foreach ($d in $CRM_Departments) { $allInAD += Get-ADUser -Filter "Department -eq '$d' -and Enabled -eq `$true" @adParams -Properties DisplayName,Department,SamAccountName }
...
Write-Log "----------------------------------------------------------------" "HEADER"
Write-Log ("  CRM-matched users in {0}   : {1}" -f $SourceGroup,$crmFiltered.Count)
