<#
.SYNOPSIS
    Syncs the Denso Service Centers Division Group AD group based on Department membership.
...
.PARAMETER Diagnose      Report Denso dept users NOT in AllEKKEmployees.
...
$TargetGroup = "Denso Service Centers Division Group"
$LogDir      = "C:\AD-MailSync\Logs"
$LogFile     = Join-Path $LogDir ("DensoEmployees_Sync_{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss"))

$Denso_Departments = @(
    "Denso Service Centre - Arad"
    "Denso Service Centre - Salmabad"
    "Denso Service Centres"
)
...
Write-Log "================================================================" "HEADER"
Write-Log "  Denso Service Centers Division Group Sync  -  $(Get-Date -Format 'dd-MMM-yyyy HH:mm')" "HEADER"
...
$densoFiltered = $allEKK | Where-Object { $dept=$_.Department; $Denso_Departments | Where-Object { $_ -ieq $dept } }
Write-Log ("Users matching Denso departments: {0}" -f $densoFiltered.Count) "INFO"
if ($densoFiltered.Count -eq 0) { Write-Log "No users matched. Verify Department values in AD." "WARN"; exit 0 }
Write-Log "Department breakdown:"
$densoFiltered | Group-Object Department | Sort-Object Count -Descending | ForEach-Object { Write-Log ("  {0,4}  {1}" -f $_.Count,$_.Name) }

Write-Log "--- Step 2: Reading current $TargetGroup members ---" "INFO"
try {
    $currentDenso = Get-ADGroupMember -Identity $TargetGroup -Recursive @adParams |
        Where-Object { $_.objectClass -eq 'user' } |
        ForEach-Object { Get-ADUser -Identity $_.DistinguishedName @adParams -Properties DisplayName,Department,EmailAddress,SamAccountName }
    $currentDNs = $currentDenso | Select-Object -ExpandProperty DistinguishedName
    Write-Log ("Current members in {0}: {1}" -f $TargetGroup, $currentDenso.Count)
} catch { Write-Log "Failed to read '$TargetGroup': $_" "ERROR"; exit 1 }

Write-Log "--- Step 3: Calculating changes ---" "INFO"
$toAdd          = $densoFiltered | Where-Object { $currentDNs -notcontains $_.DistinguishedName }
$toRemove       = $currentDenso  | Where-Object { $dept=$_.Department; -not ($Denso_Departments | Where-Object { $_ -ieq $dept }) }
$alreadyCorrect = $densoFiltered | Where-Object { $currentDNs -contains $_.DistinguishedName }
...
if ($toRemove.Count -gt 0) {
    Write-Log "--- Removing users (department no longer Denso) ---" "INFO"
...
if ($Diagnose) {
    Write-Log "================================================================" "HEADER"
    Write-Log "  DIAGNOSTIC: Denso dept users NOT in $SourceGroup" "HEADER"
    $ekk_DNs = $allEKK | Select-Object -ExpandProperty DistinguishedName
    $allInAD = @(); foreach ($d in $Denso_Departments) { $allInAD += Get-ADUser -Filter "Department -eq '$d' -and Enabled -eq `$true" @adParams -Properties DisplayName,Department,SamAccountName }
...
Write-Log "----------------------------------------------------------------" "HEADER"
Write-Log ("  Denso-matched users in {0}   : {1}" -f $SourceGroup,$densoFiltered.Count)
