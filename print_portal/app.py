...
# ── Queue status (bulk, parallel SSH) ──────────────────────────────────────────────
...
def api_status():
    ...
    # Using & + wait means all queues run simultaneously - total time ~= slowest single call.
    entries = " ".join(
    ...
    for line in out.splitlines():
        ...
        if mapped == "disabled":
            statuses[queue] = "disabled"
        elif ping_st == "down":
            statuses[queue] = "offline"
        else:
